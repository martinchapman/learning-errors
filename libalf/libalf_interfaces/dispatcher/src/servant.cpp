/* $Id: servant.cpp 1443 2011-02-28 16:22:45Z davidpiegdon $
 * vim: fdm=marker
 *
...
{{{
	...
	switch(type) {
		...
		case OBJ_LEARNING_ALGORITHM:
			...
			if(t <= ALG_NONE || t >= ALG_LAST_INVALID)
				goto bad_parameters;
			if(u <= 0)
				goto bad_parameters;

			new_id = store_object(new co_learning_algorithm( (enum learning_algorithm_type) t, u));
			break;
		case OBJ_NORMALIZER:
			if(data.size() != 1)
				goto bad_parameter_count;
