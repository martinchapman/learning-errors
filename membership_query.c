...
#define _Learn(x) {if (_Learn_idx >= mq_length || _Learn_mq[_Learn_idx] != x) __CPROVER_assume(0); _Learn_b[_Learn_idx++] = x;}   
...
#define _Learn_assert(x) if (!x ) {  if ((_Learn_idx == mq_length - 1) &&  (_Learn_mq[mq_length-1] == AlphaBetSize - 1)) {_Learn_b[_Learn_idx++] = AlphaBetSize - 1;   assert(0);}} __CPROVER_assume(0);
