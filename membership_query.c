...
// add special symbol for violating the assertion. Then, return false from within online.cpp whenever the query has that symbol not as the last one. 
// we then may post-process the final automaton to remove all such edges.
...
#define _Learn_assert(x) if (!x ) { /*_Learn(1);*/ if (_Learn_idx == mq_length) assert(0); assume(0);}
