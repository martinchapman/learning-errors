...
static void inspect(const struct list_head *head)
{
    ...
    // check traversal
    for (__CPROVER_unwind_count=1,head = head->next; __CPROVER_unwind_count < __CPROVER_unwind_limit && &node->linkage != head; ++__CPROVER_unwind_count,head = head->next);
...
static void gl_destroy()
{
    struct list_head *next;
    __CPROVER_unwind_count=0;
    while (++__CPROVER_unwind_count < __CPROVER_unwind_limit && &gl_list != (next = gl_list.next)) {
...
static bool gl_sort_pass()
{
    ...
    __CPROVER_unwind_count=0;
    while (++__CPROVER_unwind_count < __CPROVER_unwind_limit && &gl_list != (pos1 = pos0->next)) {
...
{
    __CPROVER_unwind_count=0;
    while (++__CPROVER_unwind_count < __CPROVER_unwind_limit && gl_sort_pass())
        ;
}
