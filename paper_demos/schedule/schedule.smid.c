...
const unsigned int __CPROVER_unwind_limit = 2;
...
#define EXPERIMENT_MAXPRIO 3
#define EXPERIMENT_MAX_USER_INPUTS 3
#define EXPERIMENT_MAX_PROC_PER_PRIO 3
...
int   n;
{
	...
	return NULL;
    f_ele = f_list->first;
    for (__CPROVER_unwind_count=1, i=1; __CPROVER_unwind_count < __CPROVER_unwind_limit && f_list->first && (i<n); ++__CPROVER_unwind_count, i++) /* logic error */
	f_ele = f_ele->next;
    return f_ele;
}
...
finish_all_processes()
{
    ...
    int total;
    total = num_processes;
    for (__CPROVER_unwind_count=1, i=0; __CPROVER_unwind_count < __CPROVER_unwind_limit && i<total; ++__CPROVER_unwind_count, i++)
	finish_process();
}
...
schedule(void)
{
    ...
    cur_proc = NULL;
    for (__CPROVER_unwind_count=1, i=MAXPRIO; __CPROVER_unwind_count < __CPROVER_unwind_limit && i > 0; ++__CPROVER_unwind_count, i--)
    {
...
int num_proc;
{
    ...
    queue = new_list();
    for (__CPROVER_unwind_count=1, i=0; __CPROVER_unwind_count < __CPROVER_unwind_limit && i<num_proc; ++__CPROVER_unwind_count, i++)
    {
	proc = new_process(prio);
...
char *argv[];
{
    ...
    initialize();
    for (__CPROVER_unwind_count=1, prio=MAXPRIO; __CPROVER_unwind_count < __CPROVER_unwind_limit && prio >= 1; ++__CPROVER_unwind_count, prio--)
    {
  int num_proc = nondet_int();
  __CPROVER_assume(0 <= num_proc && num_proc <= EXPERIMENT_MAX_PROC_PER_PRIO);
	//init_prio_queue(prio, atoi(argv[prio]));
  init_prio_queue(prio, num_proc);
    }
    /*for (status = fscanf(stdin, "%d", &command);
	 ((status!=EOF) && status);
	 status = fscanf(stdin, "%d", &command))*/
    int ic;
    for (__CPROVER_unwind_count=1,ic = 0; __CPROVER_unwind_count < __CPROVER_unwind_limit && ic < number_of_user_inputs; ++__CPROVER_unwind_count, ++ic)
    {
    command = nondet_int();
    __CPROVER_assume(FINISH == command || BLOCK == command || QUANTUM_EXPIRE == command || UNBLOCK == command || UPGRADE_PRIO == command || NEW_JOB == command || FLUSH == command);
	switch(command)
	{
	case FINISH:
	    finish_process();
