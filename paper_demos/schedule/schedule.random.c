...
int fscanf_command(void *handle, const char * const pattern, int *command)
{
  int nondet_value = nondet_int();
  nondet_value %= 7;
  nondet_value += 1;
  *command = nondet_value;
  return 1;
}
int fscanf_ratio(void *handle, const char * const pattern, float *ratio)
{
  int nondet_value = nondet_int();
  float *value = (float *) &nondet_value;
  *ratio = fmod(*value, 1.0f);
  return 1;
}
int fscanf_small_prio(void *handle, const char * const pattern, int *prio)
{
  int nondet_value = nondet_int();
  const int max_small_prio = MAXPRIO - 1;
  nondet_value %= max_small_prio;
  nondet_value += 1;
  *prio = nondet_value;
  return 1;
}
int fscanf_prio(void *handle, const char * const pattern, int *prio)
{
  int nondet_value = nondet_int();
  nondet_value %= MAXPRIO;
  nondet_value += 1;
  *prio = nondet_value;
  return 1;
}

void Learn_assert(int condition){}
...
char *argv[];
{
   ...
   // for (status = fscanf_command(stdin, "%d", &command);
	 //((status!=EOF) && status);
	 //status = fscanf(stdin, "%d", &command))
   //status = fscanf_command(stdin, "%d", &command))
   int cIndex;
    for (cIndex = MAXPRIO + 1; cIndex < argc; ++cIndex)
    {
    command = atoi(argv[cIndex]);
	switch(command)
	{
	case FINISH:
	    finish_process();
	...
	case UNBLOCK:
      ...
      fscanf_ratio(stdin, "%f", &ratio);
	    unblock_process(ratio);
	    break;
	case UPGRADE_PRIO:
	    //fscanf(stdin, "%d", &prio);
      fscanf_small_prio(stdin, "%d", &prio);
	    //fscanf(stdin, "%f", &ratio);
      fscanf_ratio(stdin, "%f", &ratio);
